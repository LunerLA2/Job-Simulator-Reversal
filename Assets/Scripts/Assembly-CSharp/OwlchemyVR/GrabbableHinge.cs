...
namespace OwlchemyVR
{
	...
	public class GrabbableHinge : WeakGrabbableController
	{
		public enum Axis
		{
			X = 0,
		...
		public Action<GrabbableHinge, bool> OnUpperLocked;

		public Action<GrabbableHinge, bool> OnLowerLocked;

		public Action<GrabbableHinge> OnUpperUnlocked;

		public Action<GrabbableHinge> OnLowerUnlocked;
		...
		[SerializeField]
		private bool breakFromHandWhenUpperLocked;

		[SerializeField]
		private bool breakFromHandWhenLowerLocked;
		...
		private void LockUpperInternal(bool isInitial)
		{
			...
			if (breakFromHandWhenUpperLocked && grabbable != null && grabbable.IsCurrInHand)
			{
			...
			upperLocked = true;
			lockTime = 0f;
			if (!flag && OnUpperLocked != null)
			{
				OnUpperLocked(this, isInitial);
			}
		}
		...
		private void LockLowerInternal(bool isInitial, bool suppressEvents = false)
		{
			...
			if (breakFromHandWhenLowerLocked && grabbable != null && grabbable.IsCurrInHand)
			{
				grabbable.CurrInteractableHand.TryRelease();
			}
			...
			lowerLocked = true;
			lockTime = 0f;
			if (!flag && !suppressEvents && OnLowerLocked != null)
			{
				OnLowerLocked(this, isInitial);
			}
		}
		...
		public void UnlockUpper(bool applyForce = true)
		{
			if (upperLocked)
			{
				...
				if (OnUpperUnlocked != null)
				{
					OnUpperUnlocked(this);
				}
			}
		}
		...
		public void UnlockLower(bool applyForce = true)
		{
			if (lowerLocked)
			{
				...
				if (OnLowerUnlocked != null)
				{
					OnLowerUnlocked(this);
				}
			}
		}
