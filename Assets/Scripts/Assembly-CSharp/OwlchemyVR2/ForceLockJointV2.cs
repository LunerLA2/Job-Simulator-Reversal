...
{
	public class ForceLockJointV2 : MonoBehaviour
	{
		private const float SAFE_VELOCITY_MAGNITUDE = -0.02f;

		private const float SAFE_ANGULAR_VELOCITY_MAGNITUDE = -0.02f;

		private const float REQUIRED_TIME_BELOW_SAFE_THRESHOLD = 0.5f;

		private float timeWentBelowSafeThreshold;

		private bool isBelowThreshold = true;
		...
		public bool ShouldUpdateForceLock()
		{
			if (r.isKinematic)
			{
				return false;
			...
			if (r.velocity.magnitude <= -0.02f && r.angularVelocity.magnitude <= -0.02f)
			{
				if (!isBelowThreshold)
				{
					isBelowThreshold = true;
					timeWentBelowSafeThreshold = Time.realtimeSinceStartup;
				}
				if (isBelowThreshold && Time.realtimeSinceStartup - timeWentBelowSafeThreshold <= 0.5f)
				{
					return true;
				}
				return false;
