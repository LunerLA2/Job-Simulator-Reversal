...
public class JobBoardCalibrationBrainControlledObject : BrainControlledObject
{
	private enum CalibrationState
	{
		Default = 0,
		CanPressXToOpenFloorAdjustment = 1,
		IsOnFloorAdjustmentScreen = 2
	}

	public const bool ALLOW_ALT_M_AND_N_IN_STANDALONE_BUILD = true;
	...
	[SerializeField]
	private PageData pageToShowRecalibrateButton;
	...
	private void OnEnable()
	{
		...
		instance2.OnPageComplete = (Action<PageStatusController>)Delegate.Combine(instance2.OnPageComplete, new Action<PageStatusController>(PageComplete));
	}
	...
	private void OnDisable()
	{
		...
		JobBoardManager instance2 = JobBoardManager.instance;
		instance2.OnPageComplete = (Action<PageStatusController>)Delegate.Remove(instance2.OnPageComplete, new Action<PageStatusController>(PageComplete));
	}

	private void PageShown(PageStatusController page)
	{
		if (page.Data == pageToShowRecalibrateButton)
		{
			JobBoardManager.instance.SetPSVRAdjustFloorScreenState(true);
			JobBoardManager.instance.SetPSVRFloorDetailsScreenState(false);
			calibrationState = CalibrationState.CanPressXToOpenFloorAdjustment;
		}
	}
	...
	private void PageComplete(PageStatusController page)
	{
		if (page.Data == pageToShowRecalibrateButton)
		{
			JobBoardManager.instance.SetPSVRAdjustFloorScreenState(false);
			JobBoardManager.instance.SetPSVRFloorDetailsScreenState(false);
			calibrationState = CalibrationState.Default;
			JobBoardManager.instance.HACK_TweenParentToX(0f);
			Debug.Log("Completed calibration, hiding extra screens.");
		}
	}
	...
	private void Awake()
	{
		...
		RenderTexture renderTexture = new RenderTexture(JobBoardBrainControlledObject.JOBBOARD_RENDERTEXTURE_WIDTH, Mathf.RoundToInt((float)JobBoardBrainControlledObject.JOBBOARD_RENDERTEXTURE_WIDTH / 1.5686275f), 0);
		if (JobBoardBrainControlledObject.JOBBOARD_RENDERTEXTURE_ANTIALIAS_LEVEL > 0)
		{
			renderTexture.antiAliasing = JobBoardBrainControlledObject.JOBBOARD_RENDERTEXTURE_ANTIALIAS_LEVEL;
		}
	...
	private void CheckForControllerRefs()
	{
	}

	private void GoToFloorAdjustmentDetailsPage()
	{
		calibrationState = CalibrationState.IsOnFloorAdjustmentScreen;
		JobBoardManager.instance.SetPSVRAdjustFloorScreenState(false);
		JobBoardManager.instance.SetPSVRFloorDetailsScreenState(true);
		JobBoardManager.instance.HACK_TweenParentToX(-100f);
	}
